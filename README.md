...
Thread 관리 로직은 사용자 경험과 데이터 정합성에 직접적인 영향을 주는 핵심 비즈니스 규칙이므로,  
경계값(Boundary) 중심 단위 테스트를 통해 명확하게 검증하였습니다.

테스트 클래스: `ChatServiceThreadRuleTest`

### 테스트 목적

- 스레드 재사용/신규 생성 규칙이 정확히 동작하는지 검증
- 30분 기준 경계값에서 오동작 방지
- 리팩토링 시 회귀(regression) 방지

Thread 규칙은 다음과 같습니다.

- 첫 질문 → 신규 스레드 생성
- 마지막 메시지 이후 30분 이내 → 기존 스레드 재사용
- 30분 초과 → 신규 스레드 생성

이를 기반으로 모든 케이스를 테스트 코드로 명시하였습니다.

---

### 테스트 방식

- `Clock.fixed`를 사용해 현재 시간을 고정
- Repository/AiClient는 Mock 처리
- `threadRepository.save()` 호출 횟수로 신규/재사용 여부 판단
...
- 시간 경계값(30분)에서 오차가 발생하지 않도록 명확한 테스트 작성
- 비즈니스 규칙을 문서가 아닌 코드로 명확히 표현
- Thread 정책 변경 시 테스트가 즉시 깨지도록 보호 장치 역할 수행

이 테스트를 통해 스레드 재사용/생성 로직의 안정성과 예측 가능성을 확보하였습니다.
